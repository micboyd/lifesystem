            ...
            {loading ? (
                ...
                <ul className="flex flex-col">
                    {items.map((item, i) => {
                        ...
                        return (
                            <li key={item._id} className={`flex items-center gap-4 py-3 ${i > 0 ? 'border-t border-neutral-100' : ''}`}>
                                <div className="w-16 shrink-0 text-right">
                                    <p className="text-sm font-semibold tabular-nums text-neutral-700">{item.startTime}</p>
                                    <p className="text-[11px] font-medium text-neutral-400">{dur}</p>
                                </div>
                                <span className="h-8 w-1 shrink-0 rounded-full bg-neutral-800" />
                                <span className="flex-1 truncate text-sm font-medium text-neutral-800">{item.title}</span>
                            </li>
                        )
                    })}
                </ul>
