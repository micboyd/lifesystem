...
import { computeBudgetDay, computeBudgetWeek, dayNumOf, weekStartOf, weekEndOf } from '../lib/budget'
...
function BudgetCard({
    ...
    // Daily maths (used when daily tracking is on, or for monthly header display)
    const dailyBudget = computeBudgetDay(row, entry, spends, today, excludedDates)
    const { monthlyAmount, monthlyRemaining } = dailyBudget

    // Weekly maths (used when weekly tracking is on)
    const weeklyBudget = isWeeklySpend
        ? computeBudgetWeek(row, entry, spends, today, excludedDates)
        : null

    const hasLoggedToday = spends.some((s) => s.date === today)

    // Daily panel values
    const { straightDailyRate, carry: dailyCarry, spentToday, remaining: dailyRemaining } = dailyBudget
    const todaysAllowance = straightDailyRate + dailyCarry
    const dailySpentPct =
        ...
        <div className="group flex flex-col gap-6 rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm transition-all duration-200 hover:border-neutral-300 hover:shadow-md">
                    ...
                    <div className="rounded-2xl bg-neutral-950 p-5 text-white">
                        <div className="flex items-end justify-between gap-4">
                            <div>
                                ...
                                <p className="mt-1 text-3xl font-bold font-mono tabular-nums tracking-tight">
                                    £{fmt(weeklyBudget.weeklyRate)}
                                </p>
                                <p className="mt-0.5 text-[10px] text-neutral-500">{weekLabel}</p>
                            </div>
                        ...
                        <div className="mt-4">
                            ...
                            <div className="mt-2 flex justify-between text-[11px] font-mono tabular-nums text-neutral-400">
                                <span>£{fmt(weeklyBudget.spentThisWeek)} spent this week</span>
                    ...
                    <div className="rounded-2xl bg-neutral-950 p-5 text-white">
                        <div className="flex items-end justify-between gap-4">
                            ...
                            {hasLoggedToday && (
                                <div className="text-right">
                                    ...
                                    <p className={['mt-1 text-xl font-bold font-mono tabular-nums', dailyRemaining >= 0 ? 'text-emerald-400' : 'text-red-400'].join(' ')}>
                                        £{fmt(Math.abs(dailyRemaining))}
                                        {dailyRemaining < 0 && <span className="ml-1 text-xs font-normal">over</span>}
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        {hasLoggedToday && (
                            <div className="mt-4">
                                <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/10">
                                    <div
                                        className={['h-full rounded-full transition-all duration-300', dailyRemaining >= 0 ? 'bg-emerald-400' : 'bg-red-400'].join(' ')}
                                        style={{ width: `${dailySpentPct}%` }}
                                    />
                                </div>
                                <div className="mt-2 flex justify-between text-[11px] font-mono tabular-nums text-neutral-400">
                        ...
                        {dayNum > 1 && (
                            <p className={['mt-3 break-words text-xs font-medium', dailyCarry >= 0 ? 'text-emerald-400' : 'text-red-400'].join(' ')}>
                                {dailyCarry >= 0
                                    ? `+£${fmt(dailyCarry)} carry → £${fmt(todaysAllowance)} today`
                                    : `-£${fmt(Math.abs(dailyCarry))} deficit → £${fmt(todaysAllowance)} today`}
                            </p>
                        )}
                    </div>

                    {hasLoggedToday && spentToday > straightDailyRate && (
                        <div className={['flex items-center gap-2.5 rounded-2xl px-4 py-3 text-sm font-semibold', dailyRemaining < 0 ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-700'].join(' ')}>
                            <i className="fa-solid fa-triangle-exclamation text-xs" aria-hidden="true" />
                            {dailyRemaining < 0
                                ? `Over budget by £${fmt(Math.abs(dailyRemaining))}`
            ...
            <button
                ...
                className={[
                    ...
                    isWeeklySpend || isDailySpend
                        ? 'bg-neutral-950 text-white hover:bg-neutral-800'
                        : 'bg-neutral-100 text-neutral-500 hover:bg-neutral-200 hover:text-neutral-700',
                ].join(' ')}
            >
                <i
                    className={`fa-solid fa-${isWeeklySpend || isDailySpend ? 'check' : 'toggle-off'}`}
                    aria-hidden="true"
                />
