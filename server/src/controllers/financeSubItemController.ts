...
export async function createSubItem(req: AuthRequest, res: Response) {
    ...
    const month  = typeof req.body.month  === 'string' && MONTH_RE.test(req.body.month) ? req.body.month : undefined
    ...
    if (month) query.month = month; else query.month = { $exists: false }
    const last = await FinanceSubItem.findOne(query).sort({ order: -1 })
    const order = last ? last.order + 1 : 0

    const item = await FinanceSubItem.create({ user: req.userId, row: rowId, ...(month && { month }), name, amount, order })
